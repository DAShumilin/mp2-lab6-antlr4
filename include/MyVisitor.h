...
private:
    ...
    bool isDigit(char c) {
        if (static_cast<int>(c) - 48 < 0 || static_cast<int>(c) - 48 > 9) {
            return false;
        }
        else {
            return true;
    ...
    double stod(const char* str, int n) {
        ...
        while (str[i] != '.') {
            ...
            res += static_cast<double>(static_cast<int>(str[i]) - 48);
            i++;
        }
        i++;
        ...
        while (i < n) {
            ...
            res += power * static_cast<double>(static_cast<int>(str[i]) - 48);
            i++;
        }
    ...
    std::any visitExprSUMexpr(mygrammarParser::ExprSUMexprContext* context) {
        std::cout << "Working in ExprSUMexpr" << std::endl;
